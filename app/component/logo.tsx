...
export function Logo(prop: Props) {
  const { isAbsolute = false } = prop;
  return (
    <Link to="/">
      <header
        className={`
        ${prop.isAbsolute ? "absolute" : "flex"} 
        flex items-center gap-2 pt-10 pl-8 top-0 left-0
      `}
      >
        <svg
        ...
        <h1 className="text-[#2D2926] tracking-widest uppercase font-serif">
          Bloom & Note
        </h1>
      </header>
    </Link>
